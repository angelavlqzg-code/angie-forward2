...
export async function exchangeCodeForToken(code, verifier, fetcher = fetch) {
  ...
  let data = {};
  try { data = raw ? JSON.parse(raw) : {}; } catch { /* Canva a veces regresa HTML en errores graves (dominio mal, etc.) */ }
  if (!r.ok) {
    // Se registra en los logs de Vercel (Deployments → el deploy activo → Functions/Logs →
    // busca "CANVA token exchange falló") el detalle crudo que Canva mandó — sin esto, el
    // error que ve la usuaria en el navegador queda genérico y no dice qué rechazó Canva.
    console.error("CANVA token exchange falló:", r.status, raw.slice(0, 500));
    const msg = data.error_description || data.error || `Canva respondió ${r.status} sin detalle. Revisa que el Redirect URI en Canva sea idéntico a CANVA_REDIRECT_URI en Vercel.`;
    throw new CanvaError(msg, r.status, data);
  }
...
export async function refreshAccessToken(refreshToken, fetcher = fetch) {
  ...
  const raw = await r.text();
  let data = {};
  try { data = raw ? JSON.parse(raw) : {}; } catch { /* ver nota de exchangeCodeForToken */ }
  if (!r.ok) {
    console.error("CANVA refresh token falló:", r.status, raw.slice(0, 500));
    throw new CanvaError(data.error_description || data.error || `No se pudo renovar la conexión con Canva (Canva respondió ${r.status}).`, r.status, data);
  }
  return data;
}
...
export async function createDesign({ accessToken, title, designType = "presentation", width, height, assetId }, fetcher = fetch) {
  ...
  try { data = raw ? JSON.parse(raw) : {}; } catch { /* ver nota de exchangeCodeForToken */ }
  if (!r.ok) {
    console.error("CANVA create design falló:", r.status, raw.slice(0, 500));
    throw new CanvaError(data.message || `Canva rechazó la creación del diseño (${r.status}).`, r.status, data);
  }
