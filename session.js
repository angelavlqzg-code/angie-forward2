...
//
// Tres contraseñas compartidas posibles (ANGIE_ADMIN_PASSWORD / ANGIE_ADMIN2_PASSWORD /
// ANGIE_GUEST_PASSWORD), NO cuentas individuales: quien entra con la de admin es "admin",
// quien entra con la segunda de admin es "admin2" (pedida por Angela específicamente para
// dar a Ricardo Oviedo su propio acceso, con el MISMO nivel de permisos que admin —incluye
// el Canva y el HubSpot reales y conectados de Forward, decisión explícita de Angela—, pero
// con su información del wizard/proyectos guardada aparte, para que "empiece de cero" sin
// heredar la memoria de negocio ya cargada en la cuenta admin principal), y quien entra con
// la de invitada es "invitada". La cookie guarda {role, iat} y va firmada con HMAC-SHA256
// usando ANGIE_SESSION_SECRET, para que nadie pueda escribir "role":"admin" a mano en su
// navegador y colarse — sin esa firma, el servidor rechaza la sesión.
//
...
const VALID_ROLES = ["admin", "admin2", "invitada"];
...
export function isSessionConfigured() {
  return !!(
    process.env.ANGIE_SESSION_SECRET &&
    (process.env.ANGIE_ADMIN_PASSWORD || process.env.ANGIE_ADMIN2_PASSWORD || process.env.ANGIE_GUEST_PASSWORD)
  );
}

/** Revisa la contraseña recibida contra las configuradas. Regresa el rol o null. */
export function roleForPassword(password) {
  ...
  if (process.env.ANGIE_ADMIN2_PASSWORD && password === process.env.ANGIE_ADMIN2_PASSWORD) return "admin2";
...
export function verifySession(cookieValue, secret = process.env.ANGIE_SESSION_SECRET, maxAgeSec = DEFAULT_MAX_AGE) {
  ...
  if (!data || !VALID_ROLES.includes(data.role)) return null;
